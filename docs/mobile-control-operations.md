...
| Credential or transcript disclosure | Operator and collector roles are separate. Credentials and local state must be private and outside Git. Audit/history omit instruction text, lease tokens, provider output, local paths, and provider session IDs. The local worker discards bounded provider message bodies after checking correlation metadata. | The operator UI and queue intentionally contain instruction text. Private delivery receipts temporarily contain an opaque reporting lease. A bounded response excerpt is a deliberate, separate exception: it is relayed live to connected operator dashboard tabs over `/api/instructions/preview-stream` and never written to the database, a file, or the audit/history record — see [ephemeral live preview](opencode-delivery-adapter.md#ephemeral-live-preview). Apply normal database, filesystem, and browser-access protections. |
