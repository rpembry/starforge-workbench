...
| --- | --- | --- | --- | --- | --- |
| FLOW work-item registry | `flow.open_item/show/list_items`; `ai-workbench work` | None yet (#115) | FLOW skills (#116) | Local private registry and Git-managed `TASKS.md` | Configured profile; CLI tested with synthetic roots (#110). No MCP access yet. |
| FLOW tasks and history | `flow_tasks.inspect/mutate`; `ai-workbench tasks` | None yet (#115) | FLOW skills (#116) | Local document edits and Git checkpoints; `next` is read-only | Expected revision/hash and operation ID for mutations; synthetic tests (#111/#112). No central action transition is implied. |
| FLOW code workspace | `flow_code.workspace`; `ai-workbench work prepare/resume` | None yet (#115) | FLOW resume (#116) | Explicit local worktree preparation or read-only resume; no provider launch | Bound repository in private profile; synthetic tests (#119). |
