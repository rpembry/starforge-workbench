...
- When a requested Starforge Workbench (`aiw`) operation is not directly supported by its MCP server, consider creating a GitHub feature request to add that capability for future use. Do not let filing the issue substitute for completing the current request through an authorized, safe path when one exists.
